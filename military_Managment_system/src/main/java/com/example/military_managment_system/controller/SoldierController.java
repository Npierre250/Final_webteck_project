...
public class SoldierController {
    ...
    public String userRegisterForm(@ModelAttribute("student") @Validated Soldier theSoldier, BindingResult result, @RequestParam("picture") MultipartFile file) throws ParseException, IOException {
        byte[] image= file.getBytes();
        theSoldier.setPicture(image);
    ...
    public String registerStudentInDb(@ModelAttribute("student") @Validated Soldier theSoldier, BindingResult result, @RequestParam("picture") MultipartFile file) throws ParseException, IOException {
        byte[] image= file.getBytes();
        theSoldier.setPicture(image);
        Soldier savedSoldier = soldierService.saveSoldier(theSoldier);
        if(savedSoldier != null){
    ...
    public String updateStudent(@PathVariable String studentID, @ModelAttribute("student") @Validated Soldier soldier , BindingResult result, @RequestParam("picture") MultipartFile file) throws IOException {
       ...
       Soldier exitingSoldier=soldierService.findSoldier(StudId);
        ...
        if (exitingSoldier!=null) {
            ...
            exitingSoldier.setPictureName(soldier.getPictureName());

            exitingSoldier.setRank(soldier.getRank());
            exitingSoldier.setGender(soldier.getGender());
            byte[] image= file.getBytes();
            exitingSoldier.setPicture(image);
